...
const movePromise = util.promisify(fse.move);
      ...
      try {
        ...
        if (!_.isEmpty(peerDependencies)) {
          if (this.log) {
            ...
            verbose && this.serverless.cli.log(`Adding explicit peers for dependency ${module.external}`);
          }
...
module.exports = {
  ...
  async packExternalModules() {
    ...
    const packageJsonPath = path.join(process.cwd(), packagePath);
    const scripts = this.configuration.packagerOptions.scripts || [];
    const packageScripts = scripts.reduce((__, script, index) => {
      __[`script${index}`] = script;
      return __;
    }, {});
    /**
     * Determine and create packager
     * @type {import('./packagers').Packagers}
     */
    const packager = await Packagers.get.call(this, this.configuration.packager);
    ...
    if (!_.isEmpty(packageSections)) {
      if (this.log) {
        ...
        verbose && this.serverless.cli.log(`Using package.json sections ${_.join(_.keys(packageSections), ', ')}`);
      }
    }
    ...
    if (this.log) {
      ...
      verbose && this.serverless.cli.log(`Fetch dependency graph from ${packageJsonPath}`);
    }
    ...
    const relModulePath = path.relative(compositeModulePath, path.dirname(packageJsonPath));
    addModulesToPackageJson(compositeModules, compositePackage, relModulePath);
    ...
    const exists = await pathExists(packageLockPath);
    if (exists) {
      if (this.log) {
      ...
      try {
        ...
        packageLockFile = packager.rebaseLockfile(relModulePath, packageLockFile);
    ...
    await packager.install(compositeModulePath, this.configuration.packagerOptions).then(() => {
      if (this.log) {
        this.log.verbose(`Package took [${_.now() - start} ms]`);
      } else {
        verbose && this.serverless.cli.log(`Package took [${_.now() - start} ms]`);
      }
      return null;
    });
    for (const compileStats of stats.stats) {
      ...
      if (packager.mustCopyModules) {
        await movePromise(path.join(compositeModulePath, 'node_modules'), path.join(modulePath, 'node_modules'));
      }
      ...
      if (this.log) {
        ...
        verbose && this.serverless.cli.log(`Copy modules: ${modulePath} [${_.now() - startCopy} ms]`);
      }
      ...
      if (this.log) {
        ...
        verbose && this.serverless.cli.log(`Prune: ${modulePath} [${_.now() - startPrune} ms]`);
      }
      // Prune extraneous packages - removes not needed ones
      ...
      if (this.log) {
        ...
        verbose && this.serverless.cli.log(`Run scripts: ${modulePath} [${_.now() - startRunScripts} ms]`);
      }
    }
  }
};
